...
- [x] 5. 实现 Providers KCM 工作副本、稳定 ID CRUD、单 Dialog 编辑器、校验、删除确认和模板预览；测试 Cancel 不持久化、Apply 序列化 definitions。
- [x] 6. 建立静态/XML/metadata/安装一致性/运行日志检查脚本；用官方安装命令验证运行副本，日志零 ReferenceError/TypeError，套餐行实际可见。
