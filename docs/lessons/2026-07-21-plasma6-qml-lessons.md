...
## 2026-07-22 — KCM 持久化、真实查询与桌面部署

### 8. 外层 Apply/OK 才是配置提交边界

供应商编辑器实时更新候选对象，不等于 KConfig 已保存。Plasma 的配置窗口在 Apply/OK 时
调用当前页的 `saveConfig()`，然后才读取 `cfg_*` 属性。页面缺少该回调时，仍停留在编辑页
新增的供应商不会进入 `cfg_providers`，关闭后看起来像“保存成功但数据丢失”。

**规则：** 动态子表单必须在页面 `saveConfig()` 中先校验并合并当前候选项，再交给外层
KCM 持久化；测试必须覆盖页面重建，而不只检查内存模型。

### 9. 动态表单不要在每个按键后重建 delegate

限额输入框曾在输入一个字符后失焦，根因是更新数组时替换了整个 delegate。保留稳定模型
项并只更新字段，才能维持焦点、输入法组合状态和光标位置。布局对齐也应复用同一标签宽度，
不要让每个动态表单自行计算 FormLayout twins。

### 10. “登录成功”与“额度可展示”是两条验收链

Codex 设备码授权或 MiniMax API Key 保存成功，只证明凭据可用；还必须分别验证真实端点、
响应解析、数值映射、运行时快照和 UI 渲染。失败时清空旧快照并显示错误，不能用种子数据或
上一次成功数据冒充当前额度。

### 11. 自定义脚本不能在 plasmashell 内执行

cc-switch 的 `request/extractor` 结构可以复用，但 Plasma 小部件必须把网络和 JavaScript
执行放进 C++ 管理的独立 worker，并设置 HTTPS、同源重定向、超时和大小限制。QML 只保存
定义并消费去敏快照，worker 崩溃不得带走桌面、顶栏或任务栏。

### 12. 部署完成要用证据确认

只运行安装命令不足以证明新代码生效。桌面部署至少核对：源码与安装副本哈希一致、
`plasma-plasmashell.service` 产生新 PID 且保持 active、进程语言仍为 `zh_CN.UTF-8`、重启后
KConfig 中的供应商数量和稳定 ID 不变。重启前先完成测试，重启后立即检查桌面和日志。

### 13. 配置丢失时先恢复，避免要求用户重填

恢复顺序是：正式 KConfig → 明确的备份 → 剪贴板历史/临时草稿 → 最后才询问用户。
恢复前备份整个 Plasma 配置文件；只提取目标供应商，凭据和脚本不写日志。写回后再次读取
JSON，并通过真实查询和界面数据确认恢复有效。
