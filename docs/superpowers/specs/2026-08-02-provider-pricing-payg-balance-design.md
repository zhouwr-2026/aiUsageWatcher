...
3. 错误映射（照抄 cc-switch）：
   - 401/403 → `Authentication failed (HTTP xxx)`（确定性，立即透出）
   - 其余非 2xx → `API error (HTTP xxx)`
   - 网络/超时 → 瞬时错误通道（重试保留上次成功值）
   - 响应体非法 JSON → 确定性失败
4. 解析：
   - `is_available=false` → `isValid=false` + `Insufficient balance`
...
- ProvidersConfig.qml：`credentialBackendMethod` / `syncCredentialState` 增加 deepseek 分支（`saveDeepSeekApiKey` / `clearDeepSeekApiKey` / `refreshDeepSeekUsage`）
...
- 查询失败 → `errorText` 透出（照抄 cc-switch 错误文案），面板显示错误
...
2. **JS 逻辑单测**：displayProvider 的 payg 推算（有/无充值金额、余额>充值金额的 clamp）、`totalPrice` 累计（价格+充值金额混合）
3. **QML 冒烟**：`tests/run-plasma-smoke.sh` 扩展（总价标签可见性、DeepSeek 面板元素）
