...
for tool in "$qmltestrunner" /usr/bin/kpackagetool6 /usr/bin/plasmawindowed /usr/bin/diff /usr/bin/rg; do
...
echo "[smoke] 安装: kpackagetool6 --type Plasma/Applet --upgrade package"
/usr/bin/kpackagetool6 --type Plasma/Applet --upgrade package
...
echo "[smoke] 可见组件证据: tst_fullView 断言 3 provider / 5 PlanBar"
/usr/bin/rg -q 'compare\(descendantsNamed\(fullView, "providerGroup"\)\.length, 3\)' \
    tests/tst_fullView.qml
/usr/bin/rg -q 'compare\(descendantsNamed\(fullView, "planBar"\)\.length, 5\)' \
    tests/tst_fullView.qml
...
forbidden='ReferenceError|TypeError|PlasmaCore\.Units|Error loading QML file'
