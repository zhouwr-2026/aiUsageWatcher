...
- 仓库携带 GitHub Actions 工作流 `.github/workflows/ci.yml`（Arch Linux 容器：
  cmake 构建 + ctest + `run-static-checks.sh`，`push`/`pull_request`/手动触发）。
  当前经 GitHub 镜像（`https://github.com/zhouwr-2026/aiUsageWatcher.git`）真实跑通。
- 主远端为 Gitee，**Gitee 不执行 GitHub Actions**；本地等价门禁始终可用：
  ```bash
